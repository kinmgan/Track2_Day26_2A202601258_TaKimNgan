...
def test_prosecutor_end_to_end_against_the_full_fixture_set(labelled_fixtures):
    ...
    assert report["rejected"] == 0, "prosecutor must never emit a schema-invalid or over-quota claim on its own"

    # precision perfect: it never guesses wrong when it does file
    assert report["precision"] == 1.0
    # recall perfect: all 17 classes implemented and verified
    assert report["recall"] == 1.0
    assert report["false_claim_rate"] == 0.0

    # every class: present in the fixtures and 100% verified
    for cls in CLASSES:
